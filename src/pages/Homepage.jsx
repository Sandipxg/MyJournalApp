  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3">
        {posts.map((post) => (
          <div key={post.id} className="bg-white border border-gray-200 rounded-lg px-5 py-4 shadow-sm">
            <p className="text-xs text-gray-400 mb-1">#{post.id}</p>
            <h3 className="text-base font-medium text-gray-800 capitalize">{post.title}</h3>
          </div>
        ))}
      </div>
    </div>
